// FILE: apps/storefront/src/themes/basit/app-shell/AppShell.tsx
...
import ProductFavoritesRuntime from "@/themes/basit/components/product-favorites/ProductFavoritesRuntime";
import ProductCartRuntime from "@/themes/basit/components/product-cart/ProductCartRuntime";
import ToastProvider from "./_components/ToastProvider";
import { BasitThemeRuntimeProvider } from "./theme-runtime";
...
import MalakTrackingRuntime from "@/themes/basit/tracking/MalakTrackingRuntime";
const DesktopMarketingPopupRenderer = dynamic(
  () => import("@/themes/basit/screens/_popups/DesktopMarketingPopupRenderer"),
  { ssr: false, loading: () => null },
);
const ProductQuickView = dynamic(
  () => import("@/themes/basit/components/product-quick-view/ProductQuickView"),
  {
    ssr: false,
    loading: () => null,
  },
...
const DEFAULT_STOREFRONT = {
  ...
  showReversedLogo: false,
  showReversedLogoInFooter: false,
  ...
  showPlatformCopyright: false,
  breadcrumbs: true,
  showSubCategories: true,
  subCategoriesBeforeContent: true,
...
function toProductImageFit(value: any, fallback: any) {
  ...
  if (text === "fill" || text === "full" || text === "full_image") {
    return "fill";
  }
...
function toProductCardHoverStyle(value: any, fallback: any) {
  const text = String(value ?? "").trim().toLowerCase();

  if (["always", "visible", "show", "always_show"].includes(text)) {
    return "always";
  }

  if (["hidden", "hide", "none", "disabled"].includes(text)) {
    return "hidden";
  }
        ...
        dangerouslySetInnerHTML={{
          __html: `
            .mk-route-progress {
              ...
              inset-inline: 0;
              top: 0;
              z-index: var(--mk-z-route-progress, 970);
              height: 2px;
              pointer-events: none;
              overflow: hidden;
...
export default function AppShell({
  ...
  const productCard = useMemo(() => {
    ...
    return {
      ...
      equalHeight: toBool(
        appearance.is_equal_cart_height,
        base.equalHeight,
      ),
      ...
      hoverStyle: toProductCardHoverStyle(
        appearance.hover_style,
        base.hoverStyle,
      ),
    ...
    return {
      ...
      headerStyle:
        String(
          firstDefined(
            ...
            themeHeader.headerStyle,
            themeHeader.header_style,
            base.headerStyle,
          ) ?? "floating",
      ...
      headerCorners:
        String(
          firstDefined(
            ...
            themeHeader.headerCorners,
            themeHeader.header_corners,
            base.headerCorners,
          ) ?? "rounded",
        ).toLowerCase() === "square"
          ? "square"
      ...
      sidebarBg: pickColor(
        firstDefined(
          ...
          themeHeader.sidebarBg,
          themeHeader.sidebar_bg,
        ),
        base.sidebarBg || "#ffffff",
      ),

      sidebarTextColor: pickColor(
        firstDefined(
          bootstrapAppearance.sidebar_text_color,
          bootstrapHeader.sidebar_text_color,
          themeHeader.sidebarTextColor,
          themeHeader.sidebar_text_color,
        ),
        base.sidebarTextColor || "#111111",
      ),
  ...
  const storefront = useMemo(() => {
    ...
    return {
      ...
      // الشعار البديل وإعداداته تُقرأ مرة واحدة هنا، ثم تُمرر لكل الصفحات.
      reversedLogoUrl: resolveImageUrl(
        firstDefined(
          bootstrapAppearance.reversed_logo,
          bootstrapHeader.reversed_logo,
          themeStorefront.reversedLogoUrl,
          themeStorefront.reversed_logo,
        ),
      ),

      showReversedLogo: toBool(
        firstDefined(
          bootstrapAppearance.show_reversed_logo,
          bootstrapHeader.show_reversed_logo,
          themeStorefront.showReversedLogo,
          themeStorefront.show_reversed_logo,
        ),
        base.showReversedLogo,
      ),

      showReversedLogoInFooter: toBool(
        firstDefined(
          bootstrapAppearance.show_reversed_logo_in_footer,
          bootstrapHeader.show_reversed_logo_in_footer,
          themeStorefront.showReversedLogoInFooter,
          themeStorefront.show_reversed_logo_in_footer,
        ),
        base.showReversedLogoInFooter,
      ),
      ...
      disableRightClick:
        toBool(bootstrapAppearance.content_copyright, false) ||
        toBool(bootstrapAppearance.disable_right_click, false) ||
        toBool(themeStorefront.disableRightClick, false) ||
        toBool(themeStorefront.disable_right_click, false),

      showPlatformCopyright: toBool(
        firstDefined(
          bootstrapAppearance.display_copyright,
          themeStorefront.showPlatformCopyright,
          themeStorefront.display_copyright,
        ),
        base.showPlatformCopyright,
      ),

      breadcrumbs: toBool(
        firstDefined(
          bootstrapAppearance.is_breadcrumbs,
          themeStorefront.breadcrumbs,
          themeStorefront.is_breadcrumbs,
        ),
        base.breadcrumbs,
      ),

      showSubCategories: toBool(
        firstDefined(
          bootstrapAppearance.show_sub_cats,
          themeStorefront.showSubCategories,
          themeStorefront.show_sub_cats,
        ),
        base.showSubCategories,
      ),

      subCategoriesBeforeContent: toBool(
        firstDefined(
          bootstrapAppearance.banner_after_cats,
          themeStorefront.subCategoriesBeforeContent,
          themeStorefront.banner_after_cats,
        ),
        base.subCategoriesBeforeContent,
      ),
  ...
  useEffect(() => {
    ...
    const stopImageDrag = (event: Event) => {
  ...
  useEffect(() => {
    ...
    // Merchant JavaScript never runs on authentication, account, cart or checkout routes.
    if (!customJs || isBlockedRoutePath(pathname)) return;
  ...
  const shellStyle = useMemo(() => {
    ...
    // The secondary background option was removed. All routes inherit one store background.
    const pageBgSecondary = pageBg;
    ...
    return {
      ...
      // General page surfaces follow the store background. Product cards stay independent.
      "--mk-bg-card": pageBg,
      "--mk-bg-surface": pageBg,
      "--mk-bg-soft": pageBg,
      ...
      "--mk-header-logo-height": cssPx(header.logoHeightPx, 59),
      ...
      "--mk-product-add-button-bg": productCard.addButtonBg,
      "--mk-product-add-button-color": productCard.addButtonColor,
      "--mk-product-add-button-hover-bg": productCard.addButtonBg,
      "--mk-product-add-button-hover-color": productCard.addButtonColor,
    } as CSSProperties;
  }, [
    bootstrapAppearance,
    bootstrapHeader,
  ...
  const resolvedTheme = useMemo<ThemeAdapterOutput>(
    () => ({
      ...theme,
      ui: {
        ...theme.ui,
        colors: {
          ...
          productBg: pickColor(
            firstDefined(bootstrapAppearance.product_bg, colors.productBg),
            "#ffffff",
          ),
          productPromoBg: pickColor(
            firstDefined(
              bootstrapAppearance.product_promo_bg,
              colors.productPromoBg,
            ),
            "#000000",
          ),
        },
  ...
  const runtimeValue = useMemo(
    () => ({
      theme: resolvedTheme,
      bootstrap,
      appearance: bootstrapAppearance as Record<string, any>,
      rawOptions: {
        ...(bootstrapAppearance as Record<string, any>),
        ...(((data as any)?.theme?.options || {}) as Record<string, any>),
      },
    }),
    [bootstrap, bootstrapAppearance, data, resolvedTheme],
  );

  return (
    <BasitThemeRuntimeProvider value={runtimeValue}>
    <div
      className="mk-root mk-app-shell"
      data-theme-mode={isDarkMode ? "dark" : "light"}
      data-theme-code="basit"
      data-basit-layout="unified"
      data-basit-theme={isDarkMode ? "dark" : "light"}
      ...
      data-mk-breadcrumbs={storefront.breadcrumbs ? "true" : "false"}
      data-mk-show-subcategories={
        storefront.showSubCategories ? "true" : "false"
      }
      data-mk-subcategories-before-content={
        storefront.subCategoriesBeforeContent ? "true" : "false"
      }
      data-mk-display-platform-copyright={
        storefront.showPlatformCopyright ? "true" : "false"
      }
      data-mk-equal-product-height={
        productCard.equalHeight ? "true" : "false"
      }
      data-mk-product-image-fit={productCard.imageFit}
