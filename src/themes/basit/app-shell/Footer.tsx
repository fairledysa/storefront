// FILE: apps/storefront/src/themes/basit/app-shell/Footer.tsx
"use client";
...
function footerLogoUrl(value: any, depth = 0): string {
  ...
  if (Array.isArray(value)) {
    for (const item of value) {
      const url = footerLogoUrl(item, depth + 1);
      if (url) return url;
    }
  ...
  if (typeof value === "object") {
    for (const key of [
      ...
      const url = footerLogoUrl(value?.[key], depth + 1);
      if (url) return url;
    }
  }
...
function normalizeFooterLogos(value: any): string[] {
  ...
  return source
    .map((item: unknown) => footerLogoUrl(item))
    .filter((url: string) => {
...
export default function Footer({ theme, bootstrap }: Props) {
  ...
  useEffect(() => {
    const updateScrollTopVisibility = () => {
      setScrollTopVisible(window.scrollY >= 360);
    };
  ...
  const appearanceAny: any = bootstrap?.appearance || {};
  ...
  const alternateLogoUrl = pickText(
    appearanceAny?.reversed_logo,
    headerAny?.reversed_logo,
    storefront?.reversedLogoUrl,
    storefront?.reversed_logo,
  );

  const showAlternateLogoInFooter = readBool(
    appearanceAny?.show_reversed_logo_in_footer ??
      headerAny?.show_reversed_logo_in_footer ??
      storefront?.showReversedLogoInFooter ??
      storefront?.show_reversed_logo_in_footer,
    false,
  );

  const footerLogoUrl =
    showAlternateLogoInFooter && alternateLogoUrl
      ? alternateLogoUrl
      : baseFooterLogo;
  ...
  const configuredFooterLogos = normalizeFooterLogos(
    footerOptions.footer_logos ??
      footerAny?.footer_logos ??
      footerAny?.logos ??
      storefront?.footerLogos ??
      storefront?.footer_logos,
  );
  const showFooterLogos =
    configuredFooterLogos.length > 0 ||
    readBool(footerOptions.show_footer_logos, false);
  const enableBottomNav = readBool(footerOptions.enable_bottom_nav, false);
  const showPlatformCopyright = readBool(
    appearanceAny.display_copyright ??
      storefront?.showPlatformCopyright ??
      storefront?.display_copyright,
    false,
  );
  ...
  const footerLogos =
    configuredFooterLogos.length > 0 ? configuredFooterLogos : payments;
  ...
  const scrollTopColor = pickText(
    floatingAny.scroll_top_color,
    "#111111",
  );
  ...
  const renderFloatingButtons = (side: FloatingSide) => {
    ...
    const showScrollTop =
      scrollTopEnabled && scrollTopVisible && scrollTopPosition === side;
      ...
      >
        ...
        {showScrollTop ? (
          <button
            ...
            style={
              {
                "--mk-scroll-top-bg": scrollTopColor,
              } as CSSProperties
            }
  ...
  return (
    <>
      <footer
        ...
        style={footerStyle}
        dir="rtl"
        data-footer-mode={footerIsDark ? "dark" : "light"}
      >
        <div className="mk-mfooter" aria-label="فوتر المتجر للجوال">
          ...
          {footerShowNewsletter ? (
            <div className="mk-mfooter__newsletterBlock">
              <div className="mk-mfooter__stripTitle">النشرة البريدية</div>

              <form
                className="mk-mfooter__newsletter"
          ...
          {showFooterLogos && footerLogos.length > 0 ? (
            <div className="mk-mfooter__footerLogos" aria-label="شعارات الفوتر">
              {footerLogos.slice(0, 10).map((src) => (
                <PaymentLogo key={`mfooter-logo-${src}`} src={src} />
              ))}
            </div>
          ) : null}

          {!showFooterLogos && showPayments && payments.length > 0 ? (
          ...
          <div className="mk-mfooter__legal">
            <span>© {year} {copyrightText}</span>

            {showPlatformCopyright ? (
              <span className="mk-footer__platform-credit">
                صنع بإتقان على منصة elyaia
              </span>
            ) : null}
          </div>
        </div>
        ...
        <div className="mk-footer__main">
          ...
          {showFooterLogos && footerLogos.length > 0 ? (
            <div
              className="mk-footer__container mk-footer__logosStrip"
              aria-label="شعارات الفوتر"
            >
              {footerLogos.slice(0, 12).map((src) => (
                <PaymentLogo key={`footer-logo-${src}`} src={src} />
              ))}
            </div>
          ) : null}
        </div>
        ...
        <div className="mk-footer__bottom">
          <div className="mk-footer__container mk-footer__bottomRow">
            <div className="mk-footer__meta">
              ...
              {showPlatformCopyright ? (
                <div className="mk-footer__platform-credit">
                  صنع بإتقان على منصة elyaia
                </div>
              ) : null}
