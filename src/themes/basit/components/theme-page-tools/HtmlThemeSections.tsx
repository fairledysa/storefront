...
function widthModeFromSection(section: any): "container" | "full_width" {
  ...
  const resolved =
    raw && typeof raw === "object"
      ? raw.value ?? raw.id ?? raw.key ?? raw.label ?? ""
      : raw;

  const normalized = lower(resolved);
  ...
  return (
    <>
      {sections.map((section: any, index: number) => {
        ...
        const widthMode = widthModeFromSection(section);
        const isFullWidth = widthMode === "full_width";

        return (
          <section
            key={key}
            data-theme-tool="html_content"
            data-theme-page={pageKey}
          ...
          >
            <div
              style={
                isFullWidth
                  ? { width: "100%", maxWidth: "none", paddingInline: 0 }
                  : {
                      width: "100%",
                      maxWidth: 1440,
                      marginInline: "auto",
              ...
              dangerouslySetInnerHTML={{ __html: sanitizeThemeHtml(html) }}
            />
          </section>
        );
      })}
