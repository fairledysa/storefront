// FILE: apps/storefront/src/themes/basit/screens/product/ProductScreen.tsx
"use client";
...
import ProductFloatingVideo from "./_components/ProductFloatingVideo";
...
import { useBasitThemeRuntimeOptional } from "../../app-shell/theme-runtime";
...
function normalizeProductThumbnailsPosition(
  value: any,
): "bottom" | "side" {
  const unwrap = (input: any, depth = 0): string => {
    ...
    if (Array.isArray(input)) {
      for (const item of input) {
        const resolved = unwrap(item, depth + 1);
    ...
    if (typeof input === "object") {
      for (const key of ["value", "id", "key", "name", "label", "title"]) {
        if (key in input) {
          const resolved = unwrap(input[key], depth + 1);
          if (resolved) return resolved;
        }
      }
    }
  ...
  if (
    ...
    normalized === "vertical" ||
    normalized === "right" ||
    normalized.includes("بجانب") ||
    normalized.includes("الجانب")
  ) {
    return "side";
  }
...
function readDesktopProductThumbnailsPosition(
  ...sources: any[]
): "bottom" | "side" {
  const directKeys = [
    "desktop_product_thumbnails_position",
    "desktopProductThumbnailsPosition",
  ];
  ...
  const find = (source: any, depth = 0): any => {
    ...
    for (const key of nestedKeys) {
      const found = find(source[key], depth + 1);
  ...
  for (const source of sources) {
    ...
    if (found !== undefined) {
      return normalizeProductThumbnailsPosition(found);
    }
  }

  return PRODUCT_OPTIONS_FALLBACK.desktop_product_thumbnails_position;
}
...
function normalizeProductOptions(source: any): MalakBootstrapProductOptions {
  ...
  return {
    // Mandatory in Basit; ignore stale values left by removed settings.
    show_singleSelection: true,
    ...
    desktop_product_thumbnails_position: normalizeProductThumbnailsPosition(
      options.desktop_product_thumbnails_position,
    ),

    // Kept true for compatibility; the new dropdown controls bottom vs side.
    thumbs_bottom: true,
...
export default function ProductScreen({ data }: Props) {
  const themeRuntime = useBasitThemeRuntimeOptional();
  ...
  const productOptions = normalizeProductOptions({
    ...(bootstrap?.product?.options ?? {}),
    ...(themeRuntime?.bootstrap?.product?.options ?? {}),
    desktop_product_thumbnails_position:
      readDesktopProductThumbnailsPosition(
        themeRuntime?.rawOptions,
        themeRuntime?.bootstrap?.product?.options,
        bootstrap?.product?.options,
        data?.theme?.options,
        data?.options,
  ...
  const showBreadcrumbs = readBool(
    bootstrap?.appearance?.is_breadcrumbs,
    true,
  );
  ...
  const productVideo = useMemo(() => {
    const media = Array.isArray(productVm.media) ? productVm.media : [];

    return (
      media.find((item: any) => {
        const kind = text(item?.kind ?? item?.media_kind ?? item?.type).toLowerCase();
        const url = text(item?.video_url ?? item?.url ?? item?.original_url);
        return kind === "video" && Boolean(url);
      }) ?? null
    );
  }, [productVm.media]);

  const showProductVideo = readBool(
    firstValue(
      themeRuntime?.rawOptions?.show_product_video,
      data?.theme?.options?.show_product_video,
      data?.options?.show_product_video,
    ),
    true,
  );


  const enableProductVideoShorts = readBool(
    firstValue(
      themeRuntime?.rawOptions?.enable_product_video_shorts,
      data?.bootstrap?.appearance?.enable_product_video_shorts,
      data?.appearance?.enable_product_video_shorts,
      data?.theme?.settings?.enable_product_video_shorts,
      data?.theme?.options?.enable_product_video_shorts,
      data?.settings?.enable_product_video_shorts,
      data?.options?.enable_product_video_shorts,
    ),
    false,
  );

  const showProductVideoComments = readBool(
    firstValue(
      themeRuntime?.rawOptions?.show_product_video_comments,
      data?.bootstrap?.appearance?.show_product_video_comments,
      data?.appearance?.show_product_video_comments,
      data?.theme?.settings?.show_product_video_comments,
      data?.theme?.options?.show_product_video_comments,
      data?.settings?.show_product_video_comments,
      data?.options?.show_product_video_comments,
    ),
    false,
  );

  const productShortsItems = useMemo(() => {
    const candidates = [
      {
        raw: product,
        vm: productVm,
        media: Array.isArray(productVm.media) ? productVm.media : [],
      },
    ...
    return candidates
      .map(({ raw, vm, media }: any) => {
        ...
        const id = text(vm?.id ?? raw?.id ?? raw?.product_id);
        const title = text(vm?.name ?? vm?.title ?? raw?.name ?? raw?.title) || "المنتج";
        const priceValue = firstValue(
          vm?.pricing?.finalPrice,
          vm?.pricing?.final_price,
          vm?.pricing?.price,
          vm?.price,
          vm?.sale_price,
          vm?.regular_price,
        ...
        const symbol = text(
          vm?.pricing?.currencySymbol ?? vm?.currencySymbol ?? raw?.currency_symbol ?? currencySymbol,
        );
        const price = Number.isFinite(Number(priceValue))
          ? `${Number(priceValue).toFixed(currencyDecimals)} ${symbol}`.trim()
          : "";

        const existingHref = text(
          vm?.href ?? vm?.url ?? vm?.permalink ?? raw?.href ?? raw?.url ?? raw?.permalink ?? raw?.link,
        );
        const publicNoValue = Number(
          vm?.public_no ?? vm?.publicNo ?? raw?.public_no ?? raw?.publicNo ?? raw?.product_no ?? 0,
        );
        const shortCode = text(
          vm?.short_url ?? vm?.shortUrl ?? vm?.short_code ?? vm?.shortCode ??
            raw?.short_url ?? raw?.shortUrl ?? raw?.short_code ?? raw?.shortCode,
        );
        const href = existingHref && existingHref !== "#"
          ? existingHref
          : (Number.isFinite(publicNoValue) && publicNoValue > 0) || shortCode
            ? buildStoreProductHref({
                mode,
                slugNameAr: title,
                slugNameEn: text(vm?.slug ?? raw?.slug ?? title),
                publicNo: Number.isFinite(publicNoValue) && publicNoValue > 0 ? publicNoValue : 0,
        ...
        return {
          ...
          price,
          image: text(
            vm?.imageUrl ?? vm?.image_url ?? raw?.image_url ?? raw?.thumbnail_url ?? raw?.image,
          ),
          videoSrc: text(video?.video_url ?? video?.url ?? video?.original_url),
          poster: text(video?.thumbnail_url ?? video?.thumb ?? raw?.image_url),
          rating: Number(vm?.rating?.average ?? vm?.ratingAverage ?? raw?.rating?.average ?? 0),
          reviewsCount: Number(
            vm?.reviewsSummary?.count ?? vm?.reviews_count ?? raw?.rating?.count ?? 0,
          ),
          raw,
        };
      })
  ...
  const selectedSalePrice = isVariantProduct ? variantSalePrice : productSalePrice;
  ...
  const finalPrice = isVariantProduct
    ? selectedHasDiscount
      ? selectedSalePrice
      : selectedRegularPrice
    : productFinalPrice;

  const compareAtPrice = isVariantProduct
    ? selectedHasDiscount
      ? selectedRegularPrice
      : null
  ...
  /*
   * عند تعطيل تحديث السعرين يبقى السعر العلوي على سعر المنتج الأساسي،
   * بينما يواصل شريط الشراء السفلي التحديث حسب الخيار والكمية.
   */
  const topPrice = productOptions.update_both_prices
    ? finalPrice
    : productFinalPrice ?? finalPrice;

  const topCompareAtPrice = productOptions.update_both_prices
    ? compareAtPrice
    : typeof productCompareAtPrice === "number" &&
        typeof productFinalPrice === "number" &&
        productCompareAtPrice > productFinalPrice
...
const commentSubmissionEnabled = Boolean(
  storeOptions?.reviews?.productQuestionsEnabled ?? reviewsEnabled,
);

const allowGuestComments = Boolean(
  storeOptions?.reviews?.allowGuestQuestions ??
    storeOptions?.reviews?.allowGuestComments ??
    false,
);

const allowLikes = ratingSettings.allowLikes;
...
const shouldShowProductReviews = Boolean(
  ...
  if (!hasProduct) {
    return (
      <div dir="rtl" className="mk-dproduct-state">
        تعذر تحميل المنتج
      </div>
    );
  }
  ...
  return (
    <div
      ...
      className={[
        "mk-dproduct",
        productOptions.top_details_tabs ? "mk-dproduct--top-tabs" : "",
        productOptions.show_sidebar ? "mk-dproduct--with-sidebar" : "",
      ]
        .filter(Boolean)
        .join(" ")}
    >
      ...
      />
      {showBreadcrumbs ? <ProductBreadcrumbs items={breadcrumbItems} /> : null}

      <div className="mk-dproduct-main">
        <div
          className="mk-dproduct-gallerySide"
        ...
        >
          <ProductGallery
            ...
            desktopThumbsPosition={
              productOptions.desktop_product_thumbnails_position
            }
            hideMobileThumbs={productOptions.disable_thumbs_in_mobile}
        ...
        <div className="mk-dproduct-side">
          ...
          <ProductInfo
            ...
            productOptions={{ ...productOptions, hide_top_price: true }}
            tax={productVm.tax ?? tax}
            name={productVm.name}
            subtitle={productVm.subtitle || null}
            promotionTitle={promotionTitle || null}
            price={topPrice}
            compareAtPrice={topCompareAtPrice}
          ...
          {!canAddToCart ? (
            <div className="mk-dproduct-stockAlert">{stockMessage}</div>
          ) : null}
          ...
          {productOptions.show_sticky_product ? (
            <StickyAddToCart
              ...
              stickyOnMobile={productOptions.sticky_add_to_cart}
            />
          ) : null}

          {productOptions.show_product_features && productFeatures.length ? (
            <div className="mk-dproduct-features">
              {productFeatures.map((feature, index) => (
                <div
                  key={`feature-${feature.title}-${index}`}
                  className="mk-dproduct-feature"
                >
                  {feature.icon ? (
                    <span className="mk-dproduct-feature__icon">
                      {feature.icon}
                    </span>
                  ) : null}

                  <div className="mk-dproduct-feature__body">
                    <div className="mk-dproduct-feature__title">
                      {feature.title}
                    </div>

                    {feature.subtitle ? (
                      <div className="mk-dproduct-feature__subtitle">
                        {feature.subtitle}
                      </div>
                    ) : null}
                  </div>
      ...
      {!productOptions.top_details_tabs ? (
        <div className="mk-dproduct-tabsWrap">
          <ProductTabs
            mode="details"
            productId={String(productVm.id ?? "")}
      ...
      {productOptions.top_details_tabs ? (
        <div className="mk-dproduct-detailsBeforeReviews">
          <ProductTabs
            mode="details_cards"
            productId={String(productVm.id ?? "")}
            reviewsEnabled={reviewsEnabled}
      ...
      {shouldShowProductReviews ? (
        <section
          className="mk-dproduct-reviewsStandalone"
          aria-label="الأسئلة والتقييمات"
        >
          <div dir="rtl" className="mk-ptabs">
            <div className="mk-ptabs-head">
              <div className="mk-ptabs-list">
                <span className="mk-ptabs-btn is-active">
                  الأسئلة والتقييمات ({Number(reviewsCount ?? 0)})
                </span>
...
/>
      ...
      {showProductVideo && productVideo ? (
        <ProductFloatingVideo
          src={text(
            (productVideo as any)?.video_url ??
              (productVideo as any)?.url ??
              (productVideo as any)?.original_url,
          )}
          poster={text(
            (productVideo as any)?.thumbnail_url ??
              (productVideo as any)?.thumb,
          )}
          title={productVm.name}
          shortsEnabled={enableProductVideoShorts}
          commentsEnabled={showProductVideoComments}
          commentSubmissionEnabled={commentSubmissionEnabled}
          allowGuestComments={allowGuestComments}
          questionsEnabled={questionsEnabled}
          allowGuestQuestions={allowGuestQuestions}
          allowHiddenNames={ratingSettings.allowHiddenNames}
          items={productShortsItems as any}
        />
      ) : null}
