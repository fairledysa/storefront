// FILE: apps/storefront/src/themes/basit/screens/product/_components/ProductInfo.tsx
"use client";

import { useEffect, useMemo, useState } from "react";
import { createPortal } from "react-dom";
import Link from "next/link";
import ProductCountdown from "@/themes/basit/components/product-countdown/ProductCountdown";
...
import { sanitizeThemeHtml } from "../../../utils/sanitize-theme-html";
type ProductTagItem = {
  id?: string;
  name: string;
  slug?: string | null;
    ...
    >
      ...
      {canShowPurchaseCount ? (
        <div className="mk-pinfo-purchaseCount">
          <span className="mk-pinfo-purchaseCount__icon" aria-hidden="true">🔥</span>
          <span className="mk-pinfo-purchaseCount__text">
            تم شراؤه {formatNumber(purchaseCount, 0)} مرة
          </span>
        </div>
      ) : null}
      ...
      {showTaxIncludedNote && !productOptions.hide_top_price ? (
      ...
      {hasIdentifiersBox ? (
        <div className="mk-pinfo-identityRows">
          {showSku && identifiers?.sku ? (
            <div className="mk-pinfo-identityRow">
              <strong>رقم الموديل</strong>
              <span>{identifiers.sku}</span>
            </div>
          ) : null}
          ...
          {hasWeightBox ? (
            <div className="mk-pinfo-identityRow">
              <strong>الوزن</strong>
              <span>
                {shipping?.weight} {shipping?.weight_unit || "kg"}
              </span>
            </div>
          ) : null}
        ...
        <div className="mk-pinfo-sizeGuides">
          <div className="mk-pinfo-sizeGuides__list">
            {visibleSizeGuides.map((guide) => (
              ...
              >
                <span className="mk-pinfo-sizeGuideBtn__title">دليل المقاسات</span>
                <b>عرض دليل المقاسات ‹</b>
              </button>
            ))}
          </div>
        </div>
      ...
      {sizeGuidePortalReady && activeSizeGuide
        ? createPortal(
            <div
              className="mk-root mk-sizeGuideModalPortal"
                  ...
                  {activeSizeGuide.contentHtml ? (
                    <div
                      ...
                      dangerouslySetInnerHTML={{
                        __html: sanitizeThemeHtml(activeSizeGuide.contentHtml),
                      }}
                    />
