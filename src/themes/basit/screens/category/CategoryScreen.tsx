// FILE: apps/storefront/src/themes/basit/screens/category/CategoryScreen.tsx
...
function boolValue(value: any, fallback: boolean) {
  ...
  if (typeof value === "object" && "value" in value) {
    return boolValue(value.value, fallback);
  }
...
function findCategoryInTree(rows: any[], target: any): any | null {
  const targetId = s(target?.id);
  const targetSlug = s(target?.slug);
  const targetName = s(target?.name);

  for (const row of rows) {
    const matches =
      (targetId && s(row?.id) === targetId) ||
      (targetSlug && s(row?.slug) === targetSlug) ||
      (targetName && s(row?.name) === targetName);

    if (matches) return row;

    const children = Array.isArray(row?.children) ? row.children : [];
    const nested = findCategoryInTree(children, target);
...
function resolveCategoryHref(category: any, mode: SeoUrlMode) {
  const existing = s(category?.href) || s(category?.url);
  if (existing) return existing;

  const name = s(category?.name) || "تصنيف";
  const publicNo = Number(category?.public_no ?? category?.publicNo ?? 0);

  return buildCategoryHref({
    mode,
    slugNameAr: name,
    slugNameEn: s(category?.slug) || name,
    publicNo: Number.isFinite(publicNo) ? publicNo : 0,
    shortCode: category?.short_url ?? category?.shortUrl ?? null,
  });
}

function resolveSubCategories(data: any, mode: SeoUrlMode) {
  const currentCategory = data?.category || null;
  const navigationCategories = Array.isArray(
    data?.bootstrap?.navigation?.categories,
  )
    ? data.bootstrap.navigation.categories
    : [];

  const navigationCategory = findCategoryInTree(
    navigationCategories,
    currentCategory,
  );

  const rows = Array.isArray(navigationCategory?.children)
    ? navigationCategory.children
    : Array.isArray(currentCategory?.children)
      ? currentCategory.children
      : Array.isArray(currentCategory?.subcategories)
        ? currentCategory.subcategories
        : Array.isArray(currentCategory?.sub_categories)
  ...
  return rows
    .map((category: any, index: number) => {
      const name = s(category?.name) || s(category?.title);
      if (!name) return null;

      const href = resolveCategoryHref(category, mode);
      const key = s(category?.id) || href || `${name}-${index}`;
...
export default function CategoryScreen({ data, mode }: Props) {
  ...
  const bootstrapAppearance = data?.bootstrap?.appearance || {};
  const themeOptions =
    data?.theme?.options ||
    data?.themeOptions ||
    data?.theme_options ||
  ...
  const showSubCategories = boolValue(
    firstDefined(
      themeRuntime?.theme?.storefront?.showSubCategories,
      bootstrapAppearance.show_sub_cats,
      themeOptions.show_sub_cats,
    ),
    true,
  );

  const subCategoriesBeforeContent = boolValue(
    firstDefined(
      themeRuntime?.theme?.storefront?.subCategoriesBeforeContent,
      bootstrapAppearance.banner_after_cats,
      themeOptions.banner_after_cats,
    ),
    true,
  );

  const subCategories = useMemo(
    () => resolveSubCategories(data, mode),
    [data, mode],
  );
  ...
  const subCategoriesBlock =
    !isTagPage && showSubCategories && subCategories.length > 0 ? (
      <section
        className="mk-dcat-subcats"
        aria-label={`التصنيفات الفرعية في ${pageTitle}`}
      >
        <div className="mk-dcat__container">
          <nav className="mk-dcat-subcats__list">
            {subCategories.map((category) => (
              <Link
                key={category.key}
                href={category.href}
                prefetch={false}
                className="mk-dcat-subcats__link"
              >
                <span>{category.name}</span>
                <span className="mk-dcat-subcats__arrow" aria-hidden="true">
                  ←
                </span>
              </Link>
            ))}
    ...
    return (
      <div
        ...
        className={["mk-dcat", isTagPage ? "mk-dcat--tag" : ""]
          ...
          .join(" ")}
      >
        {subCategoriesBeforeContent ? subCategoriesBlock : null}
