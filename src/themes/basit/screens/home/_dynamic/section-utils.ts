// FILE: apps/storefront/src/themes/basit/screens/home/_dynamic/section-utils.ts
...
import { isSmartSearchSection as isSmartSearchThemeSection } from "@/themes/basit/smart-search/config";
...
export function isResponsiveHeroSliderSection(section: HomeDynamicSection) {
  return (
    sectionHasToken(section, "responsive_hero_slider") ||
    sectionHasToken(section, "responsive_hero_slider_basit")
  );
}

export function isDualPromoBannerSection(section: HomeDynamicSection) {
  return (
    sectionHasToken(section, "dual_promo_banner_basit") ||
    sectionHasToken(section, "dual_promo_banner") ||
    sectionHasToken(section, "dual-promo-banner-basit")
  );
}

export function isStoryCollectionCarouselSection(
  section: HomeDynamicSection,
) {
  return (
    sectionHasToken(section, "story_collection_carousel_basit") ||
    sectionHasToken(section, "story-collection-carousel-basit") ||
    sectionHasToken(section, "story_collection_carousel")
  );
}

export function isCustomerJourneyStepsSection(
  section: HomeDynamicSection,
) {
  return (
    sectionHasToken(section, "customer_journey_steps_basit") ||
    sectionHasToken(section, "customer-journey-steps-basit") ||
    sectionHasToken(section, "customer_journey")
  );
}
...
export function isBasitCountdownOfferSection(section: HomeDynamicSection) {
  return (
    sectionHasToken(section, "countdown_offer_basit") ||
    sectionHasToken(section, "countdown-offer-basit")
  );
}
...
export function isBasitProductsTabsSection(section: HomeDynamicSection) {
  return (
    sectionHasToken(section, "products_tabs_basit") ||
    sectionHasToken(section, "products-tabs-basit")
  );
}
...
export function isBasitCircularLinksSection(section: HomeDynamicSection) {
  return (
    sectionHasToken(section, "circular_links_basit") ||
    sectionHasToken(section, "circular-links-basit")
  );
}

export function isBasitImageLinksGridSection(section: HomeDynamicSection) {
  return (
    sectionHasToken(section, "image_links_grid_basit") ||
    sectionHasToken(section, "image-links-grid-basit")
  );
}
...
export function buildDynamicSections(
  ...
  return sections
    ...
    .filter((section: HomeDynamicSection) => {
      ...
      if (isDualPromoBannerSection(section)) return true;
      if (isStoryCollectionCarouselSection(section)) return true;
      if (isCustomerJourneyStepsSection(section)) return true;
      if (isSmartSearchSection(section)) return true;

      if (isBasitCountdownOfferSection(section)) return true;
      if (isCountdownOfferSection(section)) return true;
      if (isBasitProductsTabsSection(section)) return true;
      ...
      if (isBasitCircularLinksSection(section)) return true;
      if (isBasitImageLinksGridSection(section)) return true;
...
export function getFeaturedMosaicOfferContent(
  ...
  return {
    ...
    widthMode:
      getValueText(
        values?.field_12 || values?.width_mode || values?.widthMode,
      ) === "full_width"
        ? "full_width"
        : "container",
...
export function maskCustomerName(name: string) {
  ...
  return parts
    .map((part) => {
      const firstLetter = Array.from(part)[0] || "";
...
export function normalizeTestimonialItem(
  ...
  const name =
    s(row?.customer_full_name) ||
    s(row?.customerFullName) ||
    s(row?.full_name) ||
    s(row?.fullName) ||
...
export function getTestimonialsContent(
  ...
  const nameModeRaw = lower(
    getValueText(values?.field_3 ?? values?.name_mode ?? values?.nameMode),
  );
