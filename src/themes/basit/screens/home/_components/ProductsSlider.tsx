// FILE: apps/storefront/src/themes/basit/screens/home/_components/ProductsSlider.tsx
...
export default function ProductsSlider({
  ...
  return (
    <section
      className={[
        "mk-products-slider",
        fillAvailableWidth ? "mk-products-slider--fill-width" : "",
      ]
        .filter(Boolean)
        .join(" ")}
      data-fill-width={fillAvailableWidth ? "true" : undefined}
      style={
        fillAvailableWidth
          ? ({
              "--bs-products-count-desktop": Math.max(
                1,
                Math.min(products.length, 6),
              ),
              "--bs-products-count-tablet": Math.max(
                1,
                Math.min(products.length, 4),
              ),
              "--bs-products-count-mobile": Math.max(
                1,
    ...
    >
      ...
      {products.length ? (
        products.length === 1 ? (
          <div className="mk-ps-single" data-single-product="true">
            <div
              className="mk-ps-single-card"
              data-mk-product-card-id={s(products[0]?.id)}
              onClickCapture={(event) => {
                if (!isCartClickTarget(event.target)) return;

                event.preventDefault();
                event.stopPropagation();
                dispatchAddToCart(products[0]);
              }}
            >
              <ProductCard item={products[0] as any} />
            </div>
          </div>
        ) : (
        <div className="mk-ps-shell">
          ...
          <div className="mk-ps-wrap">
            <Swiper
              ...
              spaceBetween={0}
              slidesPerView={2}
              breakpointsBase={fillAvailableWidth ? "window" : "container"}
              observer
              observeParents
              resizeObserver
              roundLengths
