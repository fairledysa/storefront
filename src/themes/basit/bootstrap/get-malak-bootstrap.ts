// FILE: apps/storefront/src/themes/basit/bootstrap/get-malak-bootstrap.ts
...
function themeOptionImageUrl(
  ...
  return resolve(findThemeOption(source, key)) || fallback;
}
...
function themeOptionProductThumbnailsPosition(
  source: Record<string, any>,
  key: string,
  fallback: "bottom" | "side",
): "bottom" | "side" {
  const value = themeOptionText(source, key, fallback).toLowerCase();
  ...
  if (
    ...
    value === "vertical" ||
    value === "right" ||
    value.includes("بجانب") ||
    value.includes("الجانب")
  ) {
    return "side";
  }
...
const customCode = sanitizeThemeCustomCode(themeOptions.custom_code);
  ...
  const appearance = {
    ...
    show_sub_cats: themeOptionBool(themeOptions, "show_sub_cats", true),
    banner_after_cats: themeOptionBool(
      themeOptions,
      "banner_after_cats",
      true,
    ),
    ...
    reversed_logo: themeOptionImageUrl(themeOptions, "reversed_logo", ""),
    ...
    // Basit removed the old "show original logo on scroll" option.
    // When the alternate logo is pinned to the home slider it stays active
    // while scrolling; inner pages continue to use the original logo.
    show_original_logo_on_scroll: false,
    ...
    // Basit has one unified appearance; dark-mode settings are intentionally disabled.
    dark_mode_switcher: false,
    dark_mode: false,

    store_bg: themeOptionText(themeOptions, "store_bg", "#ffffff"),
    // Keep the legacy key for old components, but source it from store_bg.
    store_bg_secondary: themeOptionText(themeOptions, "store_bg", "#ffffff"),
    ...
    header_style:
      themeOptionText(themeOptions, "header_style", "floating").toLowerCase() ===
      "full_width"
        ? "full_width"
        : "floating",
    header_corners:
      themeOptionText(themeOptions, "header_corners", "rounded").toLowerCase() ===
      "square"
        ? "square"
        : "rounded",
    ...
    sidebar_bg: themeOptionText(themeOptions, "sidebar_bg", "#ffffff"),
    sidebar_text_color: themeOptionText(
      themeOptions,
      "sidebar_text_color",
      "#111111",
    ),
    ...
    scroll_top_color: themeOptionText(
      themeOptions,
      "back_to_top_color",
      themeOptionText(themeOptions, "scroll_top_color", "#111111"),
    ),
    ...
    // The editor fields are stored as `logo_width` / `logo_height` in the
    // current Basit schema. Keep the old aliases as a backwards-compatible
    // fallback, but always prefer the values the merchant edits now.
    header_logo_width: themeOptionNumber(
      themeOptions,
      "logo_width",
      themeOptionNumber(themeOptions, "header_logo_width", 170, 1, 300),
      1,
      300,
    ),
    header_logo_height: themeOptionNumber(
      themeOptions,
      "logo_height",
      themeOptionNumber(themeOptions, "header_logo_height", 59, 1, 120),
      1,
      120,
    ),
    ...
    mobile_bottom_nav_style:
      themeOptionText(themeOptions, "mobile_bottom_nav_style", "solid") ===
      "frosted"
        ? "frosted"
        : "solid",
    mobile_bottom_nav_bg: themeOptionText(
      themeOptions,
      "mobile_bottom_nav_bg",
      "#ffffff",
    ),
    mobile_bottom_nav_text_color: themeOptionText(
      themeOptions,
      "mobile_bottom_nav_text_color",
      "#111111",
    ),
    ...
    footer_logos:
      themeOptions.footer_logos ??
      themeOptions.footer_logos_items ??
      themeOptions.show_footer_logos ??
      [],
  };
...
catalogFilters,
    ...
    product: {
      options: {
        // Basit: mandatory enhanced single-choice fields.
        show_singleSelection: true,
        // Basit: mandatory enhanced multiple-choice fields.
        show_multipleOption: true,

        // Basit: always show the enhanced add-to-cart confirmation.
        ...
        desktop_product_thumbnails_position:
          themeOptionProductThumbnailsPosition(
            themeOptions,
            "desktop_product_thumbnails_position",
            base.product?.options.desktop_product_thumbnails_position ??
              "bottom",
          ),

        // Legacy key kept only for compatibility with older saved versions.
        thumbs_bottom: true,
        ...
        // Basit: keep the main and purchase-bar prices synchronized.
        ...
        // Basit desktop layout always keeps product details beside the gallery.
        top_details_tabs: true,
        // Rendered automatically only when real offer data exists.
        mini_offers_box: true,

        // Rendered automatically only when store features exist.
        ...
        // The purchase box is a required part of the Basit product page.
        show_sticky_product: true,
        // Basit mobile always keeps quantity and add-to-cart fixed at the bottom.
    ...
    footer: {
      ...
      floating_actions: {
        ...
        scroll_top_color: appearance.scroll_top_color,
      ...
      options: {
        ...
        mobile_bottom_nav_style: appearance.mobile_bottom_nav_style,
        mobile_bottom_nav_bg: appearance.mobile_bottom_nav_bg,
        mobile_bottom_nav_text_color:
          appearance.mobile_bottom_nav_text_color,
        ...
        footer_logos: appearance.footer_logos,
      },
