...
function needsOnboarding(customer: any) {
  ...
  return !customer.birth_date || !customer.gender || !customer.city_id;
}
...
export default function AuthModal({
  ...
  async function syncCartAfterLogin() {
    try {
      // هذا الطلب وحده يكفي لتفعيل merge داخل getOrCreateOpenCart
  ...
  async function submitOnboarding() {
    if (!birthDate || !gender || !cityId) {
      setError("فضلاً أكمل البيانات (تاريخ الميلاد + الجنس + المدينة)");
      return;
    }
